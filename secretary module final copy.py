...
def process_arabic_text(text, is_pdf=False):
    ...
    try:
        ...
        # Enhanced Arabic text detection with more accurate thresholds
        def contains_significant_arabic(text):
            # Check if text contains meaningful Arabic content
            arabic_char_count = sum(1 for c in text if
                ('\u0600' <= c <= '\u06FF') or  # Basic Arabic
            ...
            return arabic_char_count > 0 and arabic_char_count / len(text) > 0.1  # 10% threshold
        
        # Skip processing for non-Arabic text
        if not contains_significant_arabic(text):
            return text
            
        # Create a configured reshaper instance
        try:
            from arabic_reshaper import ArabicReshaper
            
            # Enhanced configuration for patient names and medical terms
            config = {
            ...
            # Create reshaper with our configuration
            reshaper = ArabicReshaper(config)
            reshaped = reshaper.reshape(text)
            
            # For PDF contexts or forced RTL, apply bidirectional algorithm
            if is_pdf:
                return get_display(reshaped)
            else:
                # For UI contexts, just return the reshaped text
                return reshaped
                
        except ImportError:
            # Fall back to simple reshaping
...
def handle_entry_rtl(self, event, widget):
    """Handle RTL text direction for Entry widgets with improved detection"""
    text = widget.get()
    
    # Enhanced Arabic detection with broader character ranges
    def is_arabic(char):
        return ('\u0600' <= char <= '\u06FF' or  # Basic Arabic
                '\u0750' <= char <= '\u077F' or  # Arabic Supplement
                '\u08A0' <= char <= '\u08FF' or  # Arabic Extended-A
                '\uFB50' <= char <= '\uFDFF' or  # Arabic Presentation Forms-A
                '\uFE70' <= char <= '\uFEFF')    # Arabic Presentation Forms-B
    
    # Count Arabic characters for better language detection
    arabic_chars = sum(1 for c in text if is_arabic(c))
    
    # Use a threshold to determine if content should be RTL
    # For patient names, even a small amount of Arabic should trigger RTL
    if arabic_chars > 0 and arabic_chars / max(len(text), 1) > 0.05:  # 5% threshold for patient names
        # Configure for RTL
...
class MainApplication:
    def __init__(self, root):
        ...
        self.notebook.add(self.update_frame, text="Updates | التحديثات")
    ...
    def open_manual_download(self, event=None):
        """Open the manual download link in a web browser"""
        # Extract URL from label (remove emoji and label text)
        url_text = self.manual_link_var.get()
        if "://" in url_text:
            url = url_text.split("://")[1].split()[0]
            url = "https://" + url
            
            # Open URL in web browser
            webbrowser.open(url)
            
            # Update status using status_bar directly to avoid the error
            if hasattr(self, 'status_bar'):
                self.status_bar.config(text="تم فتح رابط التنزيل في المتصفح | Opened download link in browser")
    
    def setup_update_tab(self):
        """Setup the update tab for software updates"""
        # Create main frame with padding
        main_frame = ttk.Frame(self.update_frame, padding=20)
        main_frame.pack(fill=tk.BOTH, expand=True)
        
        # Add header
        header_label = ttk.Label(
            main_frame,
            text="🔄 تحديثات البرنامج | Software Updates",
            font=("Arial", 16, "bold")
        )
        ...
        ttk.Label(
            version_frame,
            text="📦 الإصدار الحالي | Current Version:",
            font=("Arial", 11)
        ).pack(side=tk.LEFT, padx=5)
        
        self.current_version_var = tk.StringVar(value="1.0.0")
        ttk.Label(
            version_frame,
        ...
        ttk.Label(
            url_frame,
            text="🌐 عنوان التحديث | Update URL:",
            font=("Arial", 11)
        ).pack(side=tk.LEFT, padx=5)
        
        self.update_url_var = tk.StringVar(value="https://clinicsoftware.example.com/updates")
        url_entry = ttk.Entry(url_frame, textvariable=self.update_url_var, width=40)
        url_entry.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        
        # Check for updates button
        check_button = ttk.Button(
            main_frame,
            text="🔍 التحقق من التحديثات | Check for Updates",
        ...
        # Status section
        status_frame = ttk.LabelFrame(main_frame, text="📋 حالة التحديث | Update Status")
        status_frame.pack(fill=tk.BOTH, expand=True, pady=15)
        
        # Status info
        self.update_status_var = tk.StringVar(value="جاهز للتحقق من التحديثات | Ready to check for updates")
        ttk.Label(
            status_frame,
        ...
        self.update_details_text.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        self.update_details_text.insert(tk.END, "تفاصيل التحديث ستظهر هنا | Update details will appear here")
        self.update_details_text.config(state=tk.DISABLED)
        
        # Download and install button (initially disabled)
        self.install_button = ttk.Button(
            main_frame,
            text="📥 تنزيل وتثبيت التحديث | Download and Install Update",
        ...
        self.manual_link_var = tk.StringVar(value="")
        self.manual_link_label = ttk.Label(
            main_frame,
            textvariable=self.manual_link_var,
            font=("Arial", 10),
            foreground="blue",
            cursor="hand2"
        )
        self.manual_link_label.pack(pady=5)
        self.manual_link_label.bind("<Button-1>", self.open_manual_download)
        
        # Add version check on startup
        self.root.after(2000, self.check_for_updates)
    
    def check_for_updates(self):
        """Check for software updates from the configured URL"""
        # Disable check button temporarily to prevent multiple requests
        for widget in self.update_frame.winfo_children():
            if isinstance(widget, ttk.Button) and "Check for Updates" in widget["text"]:
                widget.configure(state=tk.DISABLED)
                
        # Update status
        self.update_status_var.set("جارٍ التحقق من التحديثات... | Checking for updates...")
        self.progress_var.set(10)
        
        # Clear previous details
        self.update_details_text.config(state=tk.NORMAL)
        self.update_details_text.delete(1.0, tk.END)
        self.update_details_text.insert(tk.END, "جارٍ الاتصال بخادم التحديث... | Connecting to update server...")
        self.update_details_text.config(state=tk.DISABLED)
        
        # Simulate update check (in real app, this would make an HTTP request)
        # This is just a demonstration - in a real app, you'd use requests or urllib
        def perform_check():
            time.sleep(1)  # Simulate network delay
            
            try:
                # In a real app: response = requests.get(self.update_url_var.get() + "/check")
                # Simulate a response
                current_version = self.current_version_var.get()
                latest_version = "1.1.0"  # This would come from the server
                ...
                if has_update:
                    # Update available
                    self.update_status_var.set(f"تحديث متاح! | Update available! (v{latest_version})")
                    
                    # Enable install button
                    self.install_button.configure(state=tk.NORMAL)
                    
                    # Set manual download link
                    download_url = self.update_url_var.get() + "/download"
                    self.manual_link_var.set(f"🔗 تنزيل يدوي: {download_url}")
                    
                    # Show update details
                    self.update_details_text.config(state=tk.NORMAL)
                    self.update_details_text.delete(1.0, tk.END)
                    self.update_details_text.insert(tk.END, f"""🆕 تحديث جديد متاح: الإصدار {latest_version}

✨ الميزات الجديدة:
• تحسين دعم اللغة العربية
• إصلاح مشكلات في نظام المواعيد
• تحسينات في واجهة المستخدم
• أداء أفضل وإصلاحات للأخطاء

📋 تفاصيل تقنية:
• تحديث المكتبات الأساسية
• تحسينات في قاعدة البيانات
• دعم للمزيد من التقارير

📥 انقر على زر 'تنزيل وتثبيت' للحصول على أحدث إصدار.

---

🆕 New Update Available: Version {latest_version}

✨ New Features:
• Improved Arabic language support
• Fixed appointment system issues
• UI improvements
...
📥 Click 'Download and Install' to get the latest version.
""")
                    self.update_details_text.config(state=tk.DISABLED)
                else:
                    # No update available
                    self.update_status_var.set("أنت تستخدم أحدث إصدار! | You are using the latest version!")
                    
                    # Disable install button
                    self.install_button.configure(state=tk.DISABLED)
                    
                    # Clear manual download link
                    self.manual_link_var.set("")
                    
                    # Show "up to date" message
                    self.update_details_text.config(state=tk.NORMAL)
                    self.update_details_text.delete(1.0, tk.END)
                    self.update_details_text.insert(tk.END, f"""✅ أنت تستخدم أحدث إصدار: {current_version}

لا توجد تحديثات متاحة في هذا الوقت. نوصي بالتحقق من التحديثات بانتظام للحصول على أحدث الميزات والإصلاحات.
...
There are no updates available at this time. We recommend checking for updates regularly to get the latest features and fixes.
...
""")
                ...
                # Error occurred
                self.update_status_var.set("حدث خطأ أثناء التحقق من التحديثات | Error checking for updates")
    ...
    def download_and_install_update(self):
        ...
        # Update status
        self.update_status_var.set("جارٍ تنزيل التحديث... | Downloading update...")
        ...
        self.update_details_text.config(state=tk.NORMAL)
        self.update_details_text.delete(1.0, tk.END)
        self.update_details_text.insert(tk.END, "جارٍ التنزيل، يرجى الانتظار... | Downloading, please wait...")
        self.update_details_text.config(state=tk.DISABLED)
        
        # Simulate download and install process
        def perform_download():
            # Simulate download progress
            for i in range(1, 11):
                time.sleep(0.5)  # Simulate network activity
                self.progress_var.set(i * 10)
                self.update_status_var.set(f"جارٍ التنزيل... {i*10}% | Downloading... {i*10}%")
            
            # Simulate installation
            self.update_status_var.set("جارٍ تثبيت التحديث... | Installing update...")
            time.sleep(2)  # Simulate installation time
            
            # Complete the process
            self.update_status_var.set("تم التحديث بنجاح! | Update successful!")
            
            # Update version display
            self.current_version_var.set("1.1.0")
            
            # Show completion message
            self.update_details_text.config(state=tk.NORMAL)
            self.update_details_text.delete(1.0, tk.END)
            self.update_details_text.insert(tk.END, """✅ تم تحديث البرنامج بنجاح!

يرجى إعادة تشغيل التطبيق لتطبيق التغييرات الجديدة.

شكرًا لاستخدام نظام إدارة العيادة الخاص بنا!

---

✅ Software updated successfully!

Please restart the application to apply the new changes.

Thank you for using our Clinic Management System!
""")
            self.update_details_text.config(state=tk.DISABLED)
            ...
            if messagebox.askyesno(
                ...
                "يلزم إعادة التشغيل لإكمال التحديث. هل تريد إعادة التشغيل الآن؟\n\nRestart is required to complete the update. Would you like to restart now?"
            ):
                # In a real app, you would restart the application here
                self.root.destroy()  # Just close for now
    ...
    def open_manual_download(self, event=None):
        ...
        if "://" in url_text:
            ...
            # Update status
            self.set_status("تم فتح رابط التنزيل في المتصفح | Opened download link in browser")
    ...
    def initialize_database(self):
        """Initialize database connection"""
        try:
            # Connect to database with foreign key support and explicit converters for Python 3.12+ compatibility
            conn = sqlite3.connect(
                self.db_path,
                detect_types=sqlite3.PARSE_DECLTYPES
            )
            
            # Fix for Python 3.12 SQLite timestamp deprecation warning
            if sys.version_info >= (3, 12):
                # Register explicit timestamp converter function
                def convert_timestamp(val):
                    if val is None:
                        return None
                    # Convert from SQLite timestamp string to Python datetime
                    try:
...
class SecretaryModule:
    ...
    def set_status(self, message):
        ...
        try:
            # If main_app has a status_bar attribute, use it
            if hasattr(self.main_app, 'status_bar'):
                self.main_app.status_bar.config(text=message)
            # Otherwise, print to console as fallback
            else:
    ...
    def load_patients(self):
        """Load patients into the patient selection combobox with improved Arabic support"""
        try:
            ...
            for patient_id, name in patients:
                # Process Arabic text with enhanced support
                ...
                has_arabic = name and any(is_arabic(c) for c in name)
                
                # Apply Arabic text processing if needed
                if has_arabic and self.arabic_support:
                    try:
                        # Process with configuration suitable for names
                        if hasattr(self, 'arabic_reshaper') and hasattr(self, 'get_display'):
                            from arabic_reshaper import ArabicReshaper
                            config = {
                            ...
                            # If ArabicReshaper class is available, use it with config
                            try:
                                reshaper = ArabicReshaper(config)
                                reshaped_name = reshaper.reshape(name)
                            except:
                                # Fall back to basic reshaper
                                reshaped_name = self.arabic_reshaper.reshape(name)
                                
                            # Apply bidirectional algorithm for display
                            display_name = self.get_display(reshaped_name)
                            formatted_name = f"{patient_id} - {display_name}"
                    except Exception as e:
                        print(f"Error processing Arabic name: {e}")
            ...
            # Configure combobox for RTL if needed
            if any('ا' in pv or 'م' in pv or 'ح' in pv for pv in patient_values):
                self.patient_combo.configure(justify=tk.RIGHT)
    ...
    def show_daily_summary(self):
        """Show daily summary dialog with enhanced RTL support"""
        ...
        query = """
        SELECT a.id, a.start_time, a.patient_id, a.reason, a.status,
        ...
        if not appointments:
            # No appointments today - don't show dialog
            return
        
        # Create daily summary dialog
        summary_dialog = tk.Toplevel(self.parent)
        summary_dialog.title("📊 ملخص اليوم | Today's Summary")
        summary_dialog.geometry("600x400")
        summary_dialog.transient(self.parent)
        summary_dialog.grab_set()
        
        # Configure dialog
        summary_dialog.configure(background='#f5f5f5')
        summary_dialog.resizable(True, True)
        
        # Create frame with padding
        main_frame = ttk.Frame(summary_dialog, padding=10)
        main_frame.pack(fill=tk.BOTH, expand=True)
        ...
        ttk.Label(
            header_frame,
            text=f"📅 مواعيد اليوم: {day_name} | Today's Appointments: {date_str}",
            font=("Arial", 14, "bold")
        ).pack(anchor=tk.CENTER)
        
        ttk.Label(
            header_frame,
            text=f"🗓️ عدد المواعيد: {len(appointments)} | Total: {len(appointments)} appointments",
            font=("Arial", 12)
        ).pack(anchor=tk.CENTER, pady=(5, 0))
        
        # Create treeview with enhanced RTL support
        tree_frame = ttk.Frame(main_frame)
        tree_frame.pack(fill=tk.BOTH, expand=True, pady=10)
        
        # Create appointment list with columns for RTL display
        columns = ("time", "patient", "phone", "reason", "status")
        tree = ttk.Treeview(
            tree_frame,
            columns=columns,
            show="headings",
        ...
        # Configure columns with RTL-friendly anchors
        tree.heading("time", text="⏰ الوقت | Time", anchor=tk.CENTER)
        tree.heading("patient", text="👤 المريض | Patient", anchor=tk.CENTER)
        tree.heading("phone", text="📞 الهاتف | Phone", anchor=tk.CENTER)
        tree.heading("reason", text="📝 سبب الزيارة | Reason", anchor=tk.CENTER)
        tree.heading("status", text="⚑ الحالة | Status", anchor=tk.CENTER)
        
        # Set column anchors for Arabic text (right-aligned)
        tree.column("time", width=80, anchor=tk.CENTER)
        tree.column("patient", width=150, anchor=tk.E)  # Right-aligned for Arabic
        tree.column("phone", width=100, anchor=tk.CENTER)
        tree.column("reason", width=150, anchor=tk.E)  # Right-aligned for Arabic
        tree.column("status", width=100, anchor=tk.E)  # Right-aligned for Arabic
        ...
        # Configure tag styles for status colors
        for status, color in self.status_types.items():
            status_name = status.split(" | ")[0].strip()
            tree.tag_configure(status_name, background=color)
        
        # Populate treeview with appointments
        for appt in appointments:
            # Process Arabic text for display
            patient_name = appt['patient_name'] if appt['patient_name'] else ""
            reason = appt['reason'] if appt['reason'] else ""
            status = appt['status'] if appt['status'] else ""
            phone = appt['phone'] if appt['phone'] else ""
            
            # Process Arabic text with reshaping and bidirectional support
            if self.arabic_support:
                # Function to detect Arabic content
                ...
                if patient_name and contains_arabic(patient_name):
                    try:
                        from arabic_reshaper import ArabicReshaper
                        config = {
                            'delete_harakat': False,
                        ...
                        reshaper = ArabicReshaper(config)
                        patient_name = reshaper.reshape(patient_name)
                        patient_name = self.get_display(patient_name)
                    except Exception as e:
                        print(f"Error reshaping Arabic name: {e}")
                
                # Process reason
                if reason and contains_arabic(reason):
                    try:
                        from arabic_reshaper import ArabicReshaper
                        config = {
                            'delete_harakat': False,
                        ...
                        reshaper = ArabicReshaper(config)
                        reason = reshaper.reshape(reason)
                        reason = self.get_display(reason)
                    except Exception as e:
                        print(f"Error reshaping Arabic reason: {e}")
                        
                # Process status
                if status and contains_arabic(status):
                    try:
                        from arabic_reshaper import ArabicReshaper
                        config = {
                            'delete_harakat': False,
                        ...
                        reshaper = ArabicReshaper(config)
                        status = reshaper.reshape(status)
                        status = self.get_display(status)
                    except Exception as e:
                        print(f"Error reshaping Arabic status: {e}")
            
            # Get tag for status color
            status_tag = status.split(" | ")[0].strip() if " | " in status else status
            
            # Format time for display
            time_str = appt['start_time'] if appt['start_time'] else ""
            
            # Insert appointment with RTL support
            tree.insert(
                "",
                tk.END,
                values=(time_str, patient_name, phone, reason, status),
                tags=(status_tag,)
            )
        ...
        ttk.Button(
            button_frame,
            text="🖨️ طباعة | Print",
            command=lambda: self.print_daily_schedule(today)
        ).pack(side=tk.LEFT, padx=5)
        
        # SMS reminder button with RTL support
        ttk.Button(
            button_frame,
            text="📱 إرسال تذكير جماعي | Send SMS Reminder",
            command=lambda: self.send_reminders(appointments)
        ).pack(side=tk.LEFT, padx=5)
...
"""
    ...
    def update_appointments(self):
        """Update appointments list with improved Arabic support"""
        try:
            # Clear current items
            ...
            # Update date display in UI if applicable
            if hasattr(self, 'selected_date_label'):
                date_str = selected_date.strftime("%Y-%m-%d")
                day_name = selected_date.strftime("%A")
                self.selected_date_label.config(text=f"📅 {day_name} | {date_str}")
            ...
            # Get appointments for selected date, sorted by time
            query = """
            SELECT a.id, a.start_time, a.end_time, a.patient_id, a.reason, a.status,
            ...
            ORDER BY a.start_time
            """
            cursor.execute(query, (selected_date.isoformat(),))
            appointments = cursor.fetchall()
            
            # Reset appointment count if applicable
            if hasattr(self, 'today_count_var'):
                self.today_count_var.set(str(len(appointments)))
            
            # Count waiting patients
            waiting_count = 0
            for appt in appointments:
                status = appt['status'] if appt['status'] else ""
                if "حضور" in status or "Arrived" in status:
                    waiting_count += 1
                    
            # Update waiting count if applicable
            if hasattr(self, 'waiting_count_var'):
                self.waiting_count_var.set(str(waiting_count))
            
            # Populate treeview with appointments
            for appt in appointments:
                # Process time for display
                start_time = appt['start_time'] if appt['start_time'] else ""
                end_time = appt['end_time'] if appt['end_time'] else ""
                time_display = f"{start_time}-{end_time}" if end_time else start_time
                
                # Get patient name
                patient_name = appt['patient_name'] if appt['patient_name'] else ""
                
                # Process patient name for Arabic
                if patient_name and self.arabic_support:
                    ...
                    # Apply reshaping to Arabic names
                    if contains_arabic(patient_name):
                        try:
                            from arabic_reshaper import ArabicReshaper
                            config = {
                                'delete_harakat': False,      # Keep diacritics
                                'support_ligatures': True,    # Support Arabic ligatures
                                'use_unshaped_instead_of_isolated': True,  # Better handling
                                'delete_tatweel': False       # Keep tatweel character
                            }
                            reshaper = ArabicReshaper(config)
                            patient_name = reshaper.reshape(patient_name)
                ...
                # Process reason for RTL display
                reason = appt['reason'] if appt['reason'] else ""
                if reason and self.arabic_support:
                    # Check if reason contains Arabic characters
                    if any('\u0600' <= c <= '\u06FF' or
                           '\u0750' <= c <= '\u077F' or
                           '\u08A0' <= c <= '\u08FF' or
                           '\uFB50' <= c <= '\uFDFF' or
                ...
                # Get status tag for styling (first part before | character)
                status_tag = status.split(" | ")[0].strip() if " | " in status else status
                
                # Insert appointment with proper tag for status color
                self.appointments_tree.insert(
                    "",
                    tk.END,
                    values=(appt['id'], time_display, patient_name, reason, status),
                    tags=(status_tag,)
                )
                
            # Set status message
            if appointments:
                self.set_status(f"📅 {len(appointments)} مواعيد ليوم {selected_date.strftime('%Y-%m-%d')} | {len(appointments)} appointments for {selected_date.strftime('%Y-%m-%d')}")
            else:
                self.set_status(f"📅 لا توجد مواعيد ليوم {selected_date.strftime('%Y-%m-%d')} | No appointments for {selected_date.strftime('%Y-%m-%d')}")
    ...
    def search_patients(self):
        """Search for patients with enhanced Arabic support"""
        search_term = self.search_var.get()
        if not search_term or len(search_term) < 2:
            messagebox.showinfo("Search | بحث", "Please enter at least 2 characters to search | الرجاء إدخال حرفين على الأقل للبحث")
            return
        ...
        try:
            ...
            # Use normalized search term for better Arabic matching
            ...
            if not results:
                messagebox.showinfo(
                    ...
                    f"No patients found matching '{search_term}' | لم يتم العثور على مرضى مطابقين لـ '{search_term}'"
                )
                return
                
            # Show results in a new window with enhanced RTL support
            self.display_search_results(results)
            
        except Exception as e:
            messagebox.showerror("Search Error | خطأ في البحث", str(e))
            print(f"Search error: {e}")

    def display_search_results(self, results):
        """Display search results with enhanced RTL support"""
        # Create a new window for results
        results_window = tk.Toplevel(self.parent)
        results_window.title("🔍 نتائج البحث | Search Results")
        results_window.geometry("800x500")
        results_window.transient(self.parent)
        results_window.grab_set()
        
        # Apply styling to window
        results_window.configure(background='#f5f5f5')
        ...
        # Create header with RTL-friendly layout
        ttk.Label(
            header_frame,
            text=f"🔍 نتائج البحث ({len(results)}) | Search Results ({len(results)})",
            font=("Arial", 14, "bold")
        ).pack(side=tk.TOP, anchor=tk.CENTER)
        
        # Search results tree with enhanced RTL support
        tree_frame = ttk.Frame(main_frame)
        tree_frame.pack(fill=tk.BOTH, expand=True)
        
        # Create tree with proper RTL column configuration
        columns = ("id", "name", "phone", "email", "dob")
        tree = ttk.Treeview(
            tree_frame,
            columns=columns,
            show="headings",
        ...
        # Configure column headings with RTL support
        tree.heading("id", text="#", anchor=tk.CENTER)
        tree.heading("name", text="👤 اسم المريض | Patient Name", anchor=tk.CENTER)
        tree.heading("phone", text="📞 الهاتف | Phone", anchor=tk.CENTER)
        tree.heading("email", text="📧 البريد الإلكتروني | Email", anchor=tk.CENTER)
        tree.heading("dob", text="🎂 تاريخ الميلاد | Date of Birth", anchor=tk.CENTER)
        
        # Configure column display for RTL text
        tree.column("id", width=50, anchor=tk.CENTER)
        tree.column("name", width=200, anchor=tk.E)  # Right-aligned for Arabic names
        tree.column("phone", width=120, anchor=tk.CENTER)
        tree.column("email", width=180, anchor=tk.W)  # Left-aligned for email addresses
        tree.column("dob", width=120, anchor=tk.CENTER)
        ...
        # Populate tree with results and enhanced Arabic support
        for result in results:
            # Get patient data
            patient_id = result['id']
            patient_name = result['name'] if result['name'] else ""
            phone = result['phone'] if result['phone'] else ""
            email = result['email'] if result['email'] else ""
            
            # Convert date of birth to display format
            ...
            # Process Arabic text with enhanced RTL support
            if patient_name and self.arabic_support:
                # Function to detect Arabic content
                def contains_arabic(text):
                    return any('\u0600' <= c <= '\u06FF' or
            ...
            # Insert patient data into tree
            tree.insert(
                "",
                tk.END,
                values=(patient_id, patient_name, phone, email, dob)
            )
            
        # Action buttons frame with RTL-friendly layout
        button_frame = ttk.Frame(main_frame)
        button_frame.pack(fill=tk.X, pady=(10, 0))
        
        # Create buttons with RTL-friendly design
        # View button
        ttk.Button(
            button_frame,
            text="👁️ عرض معلومات المريض | View Patient",
            command=lambda: self.view_search_result(tree, results_window)
        ).pack(side=tk.LEFT, padx=5)
        
        # Book appointment button
        ttk.Button(
            button_frame,
            text="📅 حجز موعد | Book Appointment",
            command=lambda: self.book_for_search_result(tree, results_window)
        ).pack(side=tk.LEFT, padx=5)
        ...
        tree.bind("<Double-1>", lambda event: self.view_search_result(tree, results_window))
        
        # Add right-click menu for more actions
        self.setup_search_context_menu(tree, results_window)

    def setup_search_context_menu(self, tree, parent_window):
        """Setup context menu for search results with RTL support"""
        # Create context menu
        context_menu = tk.Menu(parent_window, tearoff=0)
        
        # Add menu items with RTL-friendly text
        context_menu.add_command(
            label="👁️ عرض معلومات المريض | View Patient",
            command=lambda: self.view_search_result(tree, parent_window)
        )
        context_menu.add_command(
            label="📅 حجز موعد | Book Appointment",
            command=lambda: self.book_for_search_result(tree, parent_window)
        )
        context_menu.add_separator()
        context_menu.add_command(
            label="📝 تعديل معلومات المريض | Edit Patient",
            command=lambda: self.edit_search_result(tree, parent_window)
        )
        context_menu.add_separator()
        context_menu.add_command(
            label="📊 سجل المواعيد | Appointment History",
            command=lambda: self.show_patient_history(tree)
        )
    ...
    def view_search_result(self, tree, parent_window=None):
        """View patient details from search results with RTL support"""
        # Get selected item
        selected_id = self.get_selected_patient_id(tree)
        if not selected_id:
            messagebox.showinfo(
                "Selection | تحديد",
                "Please select a patient | الرجاء تحديد مريض"
            )
            return
        ...
        try:
            ...
            # Create patient details window with RTL support
            details_window = tk.Toplevel(self.parent)
            details_window.title(f"👤 معلومات المريض | Patient Details: {patient['name']}")
            details_window.geometry("600x500")
            details_window.transient(self.parent)
            details_window.grab_set()
            
            # Apply styling
            details_window.configure(background='#f5f5f5')
            ...
            # Apply Arabic reshaping to name if needed
            if patient_name and self.arabic_support:
                def contains_arabic(text):
                    return any('\u0600' <= c <= '\u06FF' or
                              '\u0750' <= c <= '\u077F' or
            ...
            header_label = ttk.Label(
                main_frame,
                text=f"👤 معلومات المريض | Patient Details: {patient_name}",
                font=("Arial", 14, "bold"),
                anchor=tk.CENTER
            )
            header_label.pack(fill=tk.X, pady=(0, 15))
            
            # Patient details with RTL-friendly layout
            ...
            # Helper function to add a field with RTL support
            def add_field(row, label_text, value, is_rtl=False):
                """Add a field to the patient details with RTL support"""
                # Create label frame
                label_frame = ttk.Frame(details_frame)
                label_frame.grid(row=row, column=0, sticky=tk.E, padx=5, pady=5)
                
                # Create label with appropriate style
                ttk.Label(
                    label_frame,
                    text=label_text,
                    font=("Arial", 11, "bold")
                ...
                # Process Arabic text if needed
                if value and is_rtl and self.arabic_support:
                    def contains_arabic(text):
                ...
                # Create value display with appropriate anchor for RTL/LTR
                value_entry = ttk.Entry(details_frame, width=30)
                value_entry.insert(0, str(value) if value is not None else "")
                value_entry.configure(state="readonly")
                if is_rtl:
                    value_entry.configure(justify=tk.RIGHT)
                value_entry.grid(row=row, column=1, sticky=tk.W, padx=5, pady=5)
                return value_entry
            
            # Add patient fields with RTL support for Arabic content
            row = 0
            add_field(row, "🆔 المعرف | ID:", patient['id'])
            
            row += 1
            name_entry = add_field(row, "👤 الاسم | Name:", patient['name'], is_rtl=True)
            ...
            if dob:
                if isinstance(dob, str):
                    try:
                        dob_str = datetime.strptime(dob, '%Y-%m-%d').strftime('%d/%m/%Y')
                    except:
                        dob_str = dob
                elif isinstance(dob, date):
                    dob_str = dob.strftime('%d/%m/%Y')
            add_field(row, "🎂 تاريخ الميلاد | DOB:", dob_str)
            
            row += 1
            add_field(row, "⚧ الجنس | Gender:", patient['gender'], is_rtl=True)
            
            row += 1
            add_field(row, "📞 الهاتف | Phone:", patient['phone'])
            
            row += 1
            add_field(row, "📧 البريد الإلكتروني | Email:", patient['email'])
            
            row += 1
            address_frame = ttk.LabelFrame(details_frame, text="🏠 العنوان | Address")
            address_frame.grid(row=row, column=0, columnspan=2, sticky=tk.EW, padx=5, pady=5)
            
            address_text = scrolledtext.ScrolledText(address_frame, height=3, width=40, wrap=tk.WORD)
            address_value = patient['address'] if patient['address'] else ""
            
            # Process Arabic address if needed
            if address_value and self.arabic_support:
                def contains_arabic(text):
                    return any('\u0600' <= c <= '\u06FF' or
                              '\u0750' <= c <= '\u077F' or
                              '\u08A0' <= c <= '\u08FF' or
                ...
                if contains_arabic(address_value):
                    try:
                        from arabic_reshaper import ArabicReshaper
                        config = {
                            'delete_harakat': False,
                        ...
                        reshaper = ArabicReshaper(config)
                        address_value = reshaper.reshape(address_value)
                        address_value = self.get_display(address_value)
                        
                        # Configure text for RTL
                        address_text.tag_configure("rtl", justify=tk.RIGHT)
                    except Exception as e:
                        print(f"Error processing Arabic address: {e}")
            
            address_text.insert(tk.END, address_value)
            if address_value and any('\u0600' <= c <= '\u06FF' for c in address_value):
                address_text.tag_add("rtl", "1.0", tk.END)
            address_text.configure(state="disabled")
            address_text.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
            
            row += 1
            medical_frame = ttk.LabelFrame(details_frame, text="📋 السجل الطبي | Medical History")
            medical_frame.grid(row=row, column=0, columnspan=2, sticky=tk.NSEW, padx=5, pady=5)
            details_frame.rowconfigure(row, weight=1)
            
            medical_text = scrolledtext.ScrolledText(medical_frame, height=6, width=40, wrap=tk.WORD)
            medical_value = patient['medical_history'] if patient['medical_history'] else ""
            
            # Process Arabic medical history if needed
            if medical_value and self.arabic_support:
                def contains_arabic(text):
                    return any('\u0600' <= c <= '\u06FF' or
                              '\u0750' <= c <= '\u077F' or
                              '\u08A0' <= c <= '\u08FF' or
                ...
                if contains_arabic(medical_value):
                    try:
                        from arabic_reshaper import ArabicReshaper
                        config = {
                            'delete_harakat': False,
                        ...
                        reshaper = ArabicReshaper(config)
                        medical_value = reshaper.reshape(medical_value)
                        medical_value = self.get_display(medical_value)
                        
                        # Configure text for RTL
                        medical_text.tag_configure("rtl", justify=tk.RIGHT)
                    except Exception as e:
                        print(f"Error processing Arabic medical history: {e}")
            
            medical_text.insert(tk.END, medical_value)
            if medical_value and any('\u0600' <= c <= '\u06FF' for c in medical_value):
                medical_text.tag_add("rtl", "1.0", tk.END)
            medical_text.configure(state="disabled")
            medical_text.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
            
            # Button frame
            button_frame = ttk.Frame(main_frame)
            button_frame.pack(fill=tk.X, pady=(15, 0))
            
            # Actions buttons with RTL-friendly text
            ttk.Button(
                button_frame,
                text="📅 حجز موعد | Book Appointment",
                command=lambda: self.book_appointment_for_patient(selected_id, details_window)
            ).pack(side=tk.LEFT, padx=5)
            
            ttk.Button(
                button_frame,
                text="📝 تعديل | Edit",
                command=lambda: self.edit_patient(selected_id, details_window)
            ).pack(side=tk.LEFT, padx=5)
            
            ttk.Button(
                button_frame,
                text="📊 سجل المواعيد | History",
                command=lambda: self.view_patient_history(selected_id)
            ).pack(side=tk.LEFT, padx=5)
            
            ttk.Button(
                button_frame,
                text="🖨️ طباعة | Print",
                command=lambda: self.print_patient_details(patient)
            ).pack(side=tk.LEFT, padx=5)
            ...
            # If coming from search results, close the search window
            if parent_window:
                details_window.protocol("WM_DELETE_WINDOW",
                                      lambda: self.on_details_close(details_window, parent_window))
            
        except Exception as e:
            messagebox.showerror("Error | خطأ", str(e))
            print(f"Error viewing patient: {e}")
            import traceback
            traceback.print_exc()

    def get_selected_patient_id(self, tree):
        """Get the ID of the selected patient from a treeview"""
        ...
        # The ID is typically the first value
        if values and len(values) > 0:
            return values[0]
        return None

    def on_details_close(self, details_window, parent_window):
        """Handle closing of details window"""
        # Destroy details window
